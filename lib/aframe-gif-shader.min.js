/******/ (function (modules) {
	...
	/******/ [
	  ...
	  =============================================*/
					...
					if (el.error) {
						...
						if (tagName === 'video') {
							...
							message =
								'For <' +
								tagName +
								'> element, please use `aframe-html-shader`';
						}
					}
				...
				__getImageSrc: function __getImageSrc(src, cb) {
					...
					tester.addEventListener('load', function (e) {
						...
						_this.__getUnit8Array(src, function (arr) {
							...
							/* parse data */
							(0, _gifsparser.parseGIF)(
								arr,
								function (times, cnt, frames) {
									/* store data */
									var newData = {
										status: 'success',
										src: src,
				 ...
				 */
				__validateAndGetQuerySelector: function __validateAndGetQuerySelector(
					selector
				) {
					try {
				...
				if (
					...
					while (gif[pos] && gif[pos] !== 0x3b) {
						...
						if (blockId === 0x21) {
							...
							pos +=
								1 +
								+!!(gif[pos] & 0x80) * (Math.pow(2, (gif[pos] & 0x07) + 1) * 3);
							while (gif[++pos]) {
								pos += gif[pos];
							}
